...
export default function Spectator() {
  ...
  const wallColumnCount = Math.min(4, Math.max(1, Math.ceil(Math.sqrt(Math.max(visibleStreams.length, 1)))));
  ...
  const effectiveWallColumnCount = isMobileLayout ? Math.min(2, Math.max(1, visibleStreams.length || 1)) : wallColumnCount;
  ...
  const handleLocalPlaybackStep = useCallback((deltaSeconds) => {
    ...
    const stagePlayer = playerRefs.current[mainStreamId];
    const filmPlayer = playerRefs.current[`film-${mainStreamId}`];
    const referencePlayer = stagePlayer ?? filmPlayer;
    const currentTime = referencePlayer?.getCurrentTime?.();
  ...
  return (
    ...
    >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <span className={`text-[10px] font-mono uppercase tracking-wider px-2 py-1 rounded-full border ${session?.status === 'live' ? 'text-pov-success bg-pov-success/10 border-pov-success/20' : 'text-pov-warning bg-pov-warning/10 border-pov-warning/20'}`}>
                  {session?.status === 'live' ? 'Live session' : 'VOD session'}
                </span>
                <span className="text-[10px] font-mono uppercase tracking-wider px-2 py-1 rounded-full border text-pov-muted bg-pov-muted/10 border-pov-muted/20">
                  Spectator view
                </span>
              ...
              <p className="mt-1 text-xs sm:text-sm leading-relaxed text-pov-muted">
                {isRoomHeaderCollapsed
                  ...
                  : `${hostName} controls the room while you watch in read-only mode with the same synced POV layouts.`}
              </p>
            </div>
            ...
            <button
              ...
              aria-label={isRoomHeaderCollapsed ? 'Expand spectator room panel' : 'Collapse spectator room panel'}
            >
              ...
              >
                <div className="flex flex-col gap-4 pt-1">
                  <div className="hidden gap-2 sm:grid sm:grid-cols-3 xl:min-w-[360px]">
                    <InfoPill label="Session" value={session?.spectator_link || session?.id?.slice(0, 8) || 'session'} />
                    <InfoPill label="Host" value={hostName} />
                    <InfoPill label="State" value={session?.status === 'live' ? 'Watching the live room in read-only mode' : 'Watching the saved VOD session'} />
                  </div>
      ...
      {visibleStreams.length > 0 && mainStreamId && (
        <PlaybackControls
          title="Playback Controls"
          description="These controls are local to your spectator view, so you can fine-tune the current POV without affecting the host's room state."
          activeLabel={visibleStreams.find((stream) => stream.id === mainStreamId)?.display_name || 'POV'}
          ...
          {visibleStreams.length > 0 ? (
            visibleStreams.map((stream) => {
              ...
              const stageWrapperClass = isWallView
                ? `relative aspect-video overflow-hidden rounded-2xl border-2 bg-black transition-all ${
                  ...
                  {isWallView ? (
                    <>
                      <button
                        ...
                        aria-label={`Focus ${stream.display_name}`}
                      />
                ...
                <div className="max-w-3xl">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="truncate text-xl font-semibold text-white sm:text-3xl">
                      {activeMainStream?.display_name || 'POV'}
                    </span>
                    {activeMainStream && (
                      <StatusIndicators
                        stream={activeMainStream}
                        isHost={activeMainStream.user_id === session?.host_id}
                      />
                    )}
                  </div>
                </div>
          ...
          visibleStreams.map((stream) => {
            ...
            return (
              <motion.button
                ...
                } glass-card ${isMobileLayout ? 'w-[210px] min-h-full shrink-0 snap-start first:pl-0' : ''}`}
                ...
                <div className="relative aspect-video overflow-hidden bg-black">
                  <div className="pointer-events-none absolute inset-0">
                    <StreamPlayer
                      ...
                      platform={stream.platform}
                      isMain={false}
                      onReady={(player) => handlePlayerReady(`film-${stream.id}`, player)}
